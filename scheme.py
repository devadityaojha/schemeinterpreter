...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        arg = scheme_eval(first, env)
        check_procedure(arg)
        if isinstance(arg, MacroProcedure):
            return scheme_eval(arg.apply_macro(rest, env), env)
        eval_operator = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(arg, eval_operator, env)
        # END PROBLEM 5
...
def automater(fn, tail_recur = True):
    assert isinstance(fn, recursive(boolean))
    


eval_all = scheme_map(filter(lst))
...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        return None
    oper_expression = expressions
    while oper_expression.second is not nil:
        scheme_eval(oper_expression.first, env)
        oper_expression = oper_expression.second
    return scheme_eval(oper_expression.first, env, True) #True
    # END PROBLEM 8
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        try:
            ...
            raise SchemeError('mismatched number of argument values and formal parameters {0}'.format(formals))
        return child
        # END PROBLEM 11
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        args = scheme_eval(expressions.second.first, env)
        env.define(target, args)
        return target
        # END PROBLEM 6
        ...
        eval = do_lambda_form(Pair(second_target, expressions.second), env)
        env.define(first_target, eval)
        return first_target
        # END PROBLEM 10
    else:
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 9
    second_term = expressions.second
    return LambdaProcedure(formals, second_term, env)
    # END PROBLEM 9
...
def do_if_form(expressions, env):
    ...
    if scheme_truep(scheme_eval(expressions.first, env)):
        return scheme_eval(expressions.second.first, env, True) #True
    elif len(expressions) == 3:
        return scheme_eval(expressions.second.second.first, env, True) #True
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        check_form(first_binding, 2, 2)
        new_first = first_binding.first
        new_rest = scheme_eval(first_binding.second.first, env)
        values = Pair(new_rest, values)
        forms = Pair(new_first, forms)
        bindings = bindings.second
    check_formals(forms)
    return env.make_child_frame(forms, values)
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    # BEGIN Problem 21
    check_form(expressions, 2)
    first_expression = expressions.first
    if isinstance(first_expression, Pair) and scheme_symbolp(first_expression.first):
        env.define(first_expression.first, MacroProcedure(first_expression.second, expressions.second, env))
        return first_expression.first
    else:
        reject = first_expression.first if (isinstance(first_expression, Pair)) else (first_expression)
        raise SchemeError('non-symbol: {0}'.format(reject))
    # END Problem 21
...
class MuProcedure(Procedure):
    ...
    # BEGIN PROBLEM 16
    def make_call_frame(self, values, env):
        """Defining a dynamically scoped procedure"""
        return env.make_child_frame(self.formals, values)
    # END PROBLEM 16
...
def do_mu_form(expressions, env):
    ...
    # BEGIN PROBLEM 16
    second_term = expressions.second
    return MuProcedure(formals, second_term)
    # END PROBLEM 16
